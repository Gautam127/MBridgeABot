...
export interface ExecutionHandlerDependencies {
  projectRegistry?: ProjectRegistry;
  taskManager?: TaskManager;
  lockManager?: ProjectLockManager;
  processRegistry?: RuntimeProcessRegistry;
  runner?: (options: RunCommandOptions) => Promise<CommandRunResult>;
}

export function createBuildHandler(deps?: ExecutionHandlerDependencies) {
  ...
  return async (ctx: Context): Promise<void> => {
    ...
    if (!aliasArg) {
      await ctx.reply('⚠️ Usage: /build <project-alias>\n\nUse /projects to view registered aliases.');
      return;
    }
    ...
    try {
      ...
      await ctx.reply(`❌ Error initializing build task: ${err instanceof Error ? err.message : String(err)}`);
      return;
    }

    // Step 2: Immediate acknowledgment within < 2s
    await ctx.reply(formatTaskAcknowledgment('build', project.alias, taskRecord.displayId));
    ...
    try {
      ...
      if (result.timedOut) {
        taskManager.timeoutTask(taskRecord.taskId, `Build timed out after ${project.timeoutSeconds ?? 600}s`);
      ...
      if (currentTask?.state !== TaskState.CANCELLED) {
        ...
        await ctx.reply(`❌ Build execution error on "${project.alias}": ${err?.message || String(err)}`);
      }
    } finally {
